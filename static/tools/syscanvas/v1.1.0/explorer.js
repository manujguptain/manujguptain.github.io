...
function parseARXML(xmlText){
  ...
  const interfaceByPath=new Map();
  const prototypeByPath=new Map();
  const compositionNodePath=new WeakMap();
  ...
  const childByName=(node,name)=>toArray(node?.children).find(ch=>ch.localName===name)||null;
  const localChildren=(node,name)=>toArray(node?.children).filter(ch=>ch.localName===name);
  ...
  const joinPath=(base,name)=>`${base||""}/${name||""}`.replace(/\/+/g,"/");
  const refRaw=(text)=>String(text||"").trim();
  ...
  const ifacePathFromPort=(portNode)=>{
    const tref=qAll(portNode,["[DEST='SERVICE-INTERFACE']","[DEST='CLIENT-SERVER-INTERFACE']","[DEST='SENDER-RECEIVER-INTERFACE']","[DEST='MODE-SWITCH-INTERFACE']","[DEST='NV-DATA-INTERFACE']"])[0];
    return refRaw(tref?.textContent||"");
  };
  const componentForRef=(text)=>{
    const raw=refRaw(text);
    const byPath=componentByPath.get(raw)||prototypeByPath.get(raw);
    if(byPath)return byPath;
    return componentByName.get(refTail(raw))||null;
  };
  const resolveEndpoint=(irefNode,containerNode)=>{
    const pick=(n,names)=>qAll(n,names)[0]?.textContent||"";
    const cRef=pick(irefNode||containerNode,["CONTEXT-COMPONENT-REF","CONTEXT-COMPONENT-PROTOTYPE-REF","COMPONENT-PROTOTYPE-REF","COMPONENT-REF"]);
    const pRef=pick(irefNode||containerNode,["TARGET-P-PORT-REF","TARGET-PORT-REF","PORT-PROTOTYPE-REF","P-PORT-PROTOTYPE-REF"]);
    const rRef=pick(irefNode||containerNode,["TARGET-R-PORT-REF","TARGET-PORT-REF","PORT-PROTOTYPE-REF","R-PORT-PROTOTYPE-REF"]);
    const portRef=pRef||rRef||pick(irefNode||containerNode,["TARGET-PR-PORT-REF","PR-PORT-PROTOTYPE-REF"]);
    const comp=componentForRef(cRef);
    const portName=refTail(portRef||"");
    let ifacePath="",iface="";
    if(comp){
      const prov=comp.provided.find(p=>p.name.endsWith(portName));
      const req=comp.required.find(p=>p.name.endsWith(portName));
      ifacePath=prov?.ifacePath||req?.ifacePath||"";
      iface=prov?.iface||req?.iface||"";
    }
    if(!ifacePath&&portRef){
      const maybeIface=interfaceByPath.get(refRaw(portRef));
      if(maybeIface){ifacePath=maybeIface.path;iface=maybeIface.name;}
    }
    return{
      component:comp?.name||refTail(cRef||""),
      componentPath:comp?.path||refRaw(cRef||""),
      port:portName,
      iface:iface||refTail(ifacePath||""),
      ifacePath
    };
  };

  const ifaceTags=new Set(["SERVICE-INTERFACE","CLIENT-SERVER-INTERFACE","SENDER-RECEIVER-INTERFACE","MODE-SWITCH-INTERFACE","NV-DATA-INTERFACE"]);
  const componentTags=new Set(["ADAPTIVE-APPLICATION-SW-COMPONENT-TYPE","APPLICATION-SW-COMPONENT-TYPE","COMPOSITION-SW-COMPONENT-TYPE","SERVICE-SW-COMPONENT-TYPE","SENSOR-ACTUATOR-SW-COMPONENT-TYPE","COMPLEX-DEVICE-DRIVER-SW-COMPONENT-TYPE","ECU-ABSTRACTION-SW-COMPONENT-TYPE"]);
  const walkArPackage=(pkgNode,basePath)=>{
    const pkgName=shortName(pkgNode);
    if(!pkgName)return;
    const pkgPath=joinPath(basePath,pkgName);
    const elements=childByName(pkgNode,"ELEMENTS");
    if(elements){
      toArray(elements.children).forEach(node=>{
        const tag=node.localName;
        const name=shortName(node);
        if(!name)return;
        const path=joinPath(pkgPath,name);
        if(ifaceTags.has(tag)){
          if(interfaceByPath.has(path))return;
          if(!interfaceNames.has(name))interfaceNames.add(name);
          const entry={name,path,type:tag,methods:[],events:[],fields:[]};
          ...
          interfaceByPath.set(path,entry);
          return;
        }
        if(componentTags.has(tag)){
          if(componentByPath.has(path))return;
          const comp={name,path,type:tag,isInstance:false,provided:[],required:[],arxml:(new XMLSerializer).serializeToString(node)};
          qAll(node,["P-PORT-PROTOTYPE","PROVIDED-PORT-PROTOTYPE"]).forEach(port=>{
            ...
            comp.provided.push({name:pn,iface,ifacePath:ifacePathFromPort(port)});
          });
          qAll(node,["R-PORT-PROTOTYPE","REQUIRED-PORT-PROTOTYPE"]).forEach(port=>{
            const pn=shortName(port)||"port";
            const iface=ifaceFromPort(port);
            comp.required.push({name:pn,iface,ifacePath:ifacePathFromPort(port)});
          });
          qAll(node,["PR-PORT-PROTOTYPE"]).forEach(port=>{
            const pn=shortName(port)||"port";
            const iface=ifaceFromPort(port);
            const ifacePath=ifacePathFromPort(port);
            comp.provided.push({name:pn,iface,ifacePath});
            comp.required.push({name:pn,iface,ifacePath});
          });
          components.push(comp);
          componentByName.set(name,comp);
          componentByPath.set(path,comp);
          if(tag==="COMPOSITION-SW-COMPONENT-TYPE")compositionNodePath.set(node,path);
        }
      });
    }
    localChildren(pkgNode,"AR-PACKAGES").forEach(packagesNode=>{
      localChildren(packagesNode,"AR-PACKAGE").forEach(childPkg=>walkArPackage(childPkg,pkgPath));
    });
  };
  qAll(doc,["AR-PACKAGE"]).forEach(pkg=>{const parentPackage=pkg.closest("AR-PACKAGE");if(!parentPackage)walkArPackage(pkg,"")});
  ...
  qAll(doc,["SW-COMPONENT-PROTOTYPE"]).forEach(proto=>{
    ...
    const typePath=refRaw(tref?.textContent||"");
    const typeComp=componentByPath.get(typePath)||componentByName.get(refTail(typePath));
    if(!instName||!typeComp)return;
    const parentComp=proto.closest("COMPOSITION-SW-COMPONENT-TYPE");
    const parentPath=parentComp?compositionNodePath.get(parentComp)||"":"";
    const instancePath=joinPath(parentPath||typeComp.path,instName);
    const instComp={
      name:instName,
      path:instancePath,
      type:typeComp.type,
      typeRef:typeComp.path,
      isInstance:true,
      provided:typeComp.provided.map(p=>({name:`${instName}.${p.name}`,iface:p.iface,ifacePath:p.ifacePath})),
      required:typeComp.required.map(p=>({name:`${instName}.${p.name}`,iface:p.iface,ifacePath:p.ifacePath})),
      arxml:typeComp.arxml
    };
    components.push(instComp);
    componentByPath.set(instancePath,instComp);
    prototypeByPath.set(instancePath,instComp);
    if(!componentByName.has(instName))componentByName.set(instName,instComp);
  });

  const connections=[];
  const dedupe=new Set();
  const pushConnection=(from,to,label,typeHint,origin,meta={})=>{
    if(!from||!to||from===to)return;
    const key=`${from}>${to}:${label}`;
    if(dedupe.has(key))return;
    dedupe.add(key);
    connections.push({from,to,label:label||"interface",type:guessType(typeHint||label||""),origin,meta});
  };

  const connectorNodes=qAll(doc,["ASSEMBLY-SW-CONNECTOR","ASSEMBLY-CONNECTOR-PROTOTYPE","DELEGATION-SW-CONNECTOR","DELEGATION-CONNECTOR-PROTOTYPE"]);
  connectorNodes.forEach(conn=>{
    const tag=conn.localName||"CONNECTOR";
    const providerIRef=qAll(conn,["PROVIDER-IREF","INNER-PORT-IREF"])[0]||conn;
    const requesterIRef=qAll(conn,["REQUESTER-IREF","OUTER-PORT-IREF"])[0]||conn;
    const left=resolveEndpoint(providerIRef,conn);
    const right=resolveEndpoint(requesterIRef,conn);
    const label=left.iface||right.iface||left.port||right.port||"connectorLink";
    const from=left.component;
    const to=right.component;
    pushConnection(from,to,label,label,"explicit",{connectorType:tag});
  });
  ...
  if(!connectorNodes.length){
    components.forEach(src=>src.provided.forEach(p=>{
      components.forEach(dst=>{
        if(src===dst)return;
        dst.required.forEach(r=>{if(p.iface&&r.iface&&p.iface===r.iface)pushConnection(src.name,dst.name,p.iface,p.iface,"inferred",{reason:"interface-match"})});
      });
    }));
  }
