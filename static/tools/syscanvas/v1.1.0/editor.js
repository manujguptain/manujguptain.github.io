...
function deterministicNameRegistry(){return{seen:new Map(),unique:function(e){var n=safeEsc(e||"Item")||"Item",t=this.seen.get(n)||0;return this.seen.set(n,t+1),t?`${n}_${t+1}`:n}}}
function collectInterfaceDefinitions(){
  var map=new Map(),warnings=[];
  function upsert(name,type,source){
    if(!name)return;
    var cur=map.get(name);
    if(!cur){map.set(name,{name:name,type:type,explicit:source!=="inferred",source:source});return}
    if(source!=="inferred"){
      if(cur.explicit&&cur.type!==type)warnings.push(`Interface "${name}" has conflicting explicit types (${cur.type} vs ${type}).`);
      cur.type=type;cur.explicit=!0;cur.source=source;return;
    }
    if(!cur.explicit&&!cur.type)cur.type=type;
  }
  allComps().forEach(function(c){
    c.provided.forEach(function(p){if(!p.iface)return;var t=p.ifaceType||p.type||null;if(t)upsert(p.iface,t,"port");else upsert(p.iface,guessIfaceType(p.iface),"inferred")});
    c.required.forEach(function(p){if(!p.iface)return;var t=p.ifaceType||p.type||null;if(t)upsert(p.iface,t,"port");else upsert(p.iface,guessIfaceType(p.iface),"inferred")});
  });
  connections.forEach(function(c){if(c.label)upsert(c.label,c.type||guessIfaceType(c.label),c.type?"connection":"inferred")});
  return{map:map,warnings:warnings};
}
function validateExportModel(profile){
  var warnings=[],errors=[],registry=deterministicNameRegistry(),comps=allComps().slice().sort(function(a,b){return a.name.localeCompare(b.name)||a.id.localeCompare(b.id)}),ifaceDefs=collectInterfaceDefinitions();
  warnings=warnings.concat(ifaceDefs.warnings);
  var ifaceEntries=Array.from(ifaceDefs.map.values()).sort(function(a,b){return a.name.localeCompare(b.name)}).map(function(i){return{name:i.name,type:i.type||guessIfaceType(i.name),short:registry.unique(i.name)}}),ifaceByName=new Map();
  ifaceEntries.forEach(function(i){ifaceByName.set(i.name,i)});
  var compNameReg=deterministicNameRegistry(),portNameRegByComp=new Map(),compEntries=comps.map(function(c){
    var entry={id:c.id,comp:c,short:compNameReg.unique(c.name),ports:{provided:[],required:[]}};
    var portReg=deterministicNameRegistry();portNameRegByComp.set(c.id,portReg);
    c.provided.forEach(function(p){entry.ports.provided.push({src:p,short:portReg.unique(p.name),iface:p.iface||""})});
    c.required.forEach(function(p){entry.ports.required.push({src:p,short:portReg.unique(p.name),iface:p.iface||""})});
    return entry;
  }),compById=new Map();
  compEntries.forEach(function(c){compById.set(c.id,c)});
  var connectorReg=deterministicNameRegistry(),connEntries=connections.slice().sort(function(a,b){
    var af=findComp(a.from),at=findComp(a.to),bf=findComp(b.from),bt=findComp(b.to);
    var ak=`${(af&&af.comp.name)||a.from}|${(at&&at.comp.name)||a.to}|${a.label||''}`;
    var bk=`${(bf&&bf.comp.name)||b.from}|${(bt&&bt.comp.name)||b.to}|${b.label||''}`;
    return ak.localeCompare(bk);
  }).map(function(k){
    var src=compById.get(k.from),dst=compById.get(k.to);
    if(!src||!dst){errors.push(`Connection ${k.label||"(unnamed)"} references missing component(s).`);return null}
    var iface=ifaceByName.get(k.label||"");
    if(k.label&&!iface){warnings.push(`Connection "${k.label}" has no interface definition; inferred during export.`);iface={name:k.label,type:k.type||guessIfaceType(k.label),short:registry.unique(k.label)};ifaceByName.set(k.label,iface);ifaceEntries.push(iface)}
    var srcPort=src.ports.provided.find(function(p){return p.iface===k.label})||src.ports.provided[0],dstPort=dst.ports.required.find(function(p){return p.iface===k.label})||dst.ports.required[0];
    if(!srcPort||!dstPort){errors.push(`Connection "${k.label||"(unnamed)"}" cannot be mapped to source/target ports.`);return null}
    return{k:k,short:connectorReg.unique(`Conn_${src.short}_${dst.short}_${k.label||"link"}`),src:src,dst:dst,srcPort:srcPort,dstPort:dstPort,iface:iface};
  }).filter(Boolean);
  compEntries.forEach(function(c){
    c.ports.provided.concat(c.ports.required).forEach(function(p){if(p.iface&&!ifaceByName.get(p.iface))warnings.push(`Port "${c.comp.name}/${p.short}" references unknown interface "${p.iface}".`)})
  });
  return{warnings:warnings,errors:errors,ifaceEntries:ifaceEntries.sort(function(a,b){return a.short.localeCompare(b.short)}),compEntries:compEntries,connEntries:connEntries,profile:profile};
}
function buildPackageTemplates(m){
  var root=safeEsc(m.profile.rootPackage),ifacePkg=`/${root}/Interfaces`,typesPkg=`/${root}/ComponentTypes`,compPkg=`/${root}/${safeEsc(m.profile.compositionPackage)}`,depPkg=`/${root}/Deployments`;
  return{root:root,interfaces:ifacePkg,componentTypes:typesPkg,compositions:compPkg,deployments:depPkg};
}
function generateARXML(){
  var profile=EXPORT_PROFILES[currentExportProfile]||EXPORT_PROFILES.demo,model=validateExportModel(profile),pkgs=buildPackageTemplates(model),ifaceXml="",compTypeXml="",prototypeXml="",connectorXml="",deploymentXml="";
  lastExportWarnings=model.warnings.slice();
  model.ifaceEntries.forEach(function(i){
    if(i.type==='event')ifaceXml+=`            <SENDER-RECEIVER-INTERFACE>
              <SHORT-NAME>${i.short}</SHORT-NAME>
              <DATA-ELEMENTS>
                <DATA-ELEMENT-PROTOTYPE><SHORT-NAME>${i.short}_Data</SHORT-NAME></DATA-ELEMENT-PROTOTYPE>
              </DATA-ELEMENTS>
            </SENDER-RECEIVER-INTERFACE>
`;
    else if(i.type==='field')ifaceXml+=`            <SERVICE-INTERFACE>
              <SHORT-NAME>${i.short}</SHORT-NAME>
              <FIELDS>
                <FIELD><SHORT-NAME>${i.short}_Field</SHORT-NAME></FIELD>
              </FIELDS>
            </SERVICE-INTERFACE>
`;
              ...
              <SHORT-NAME>${i.short}</SHORT-NAME>
              <OPERATIONS>
                <OPERATION-PROTOTYPE><SHORT-NAME>${i.short}_Op</SHORT-NAME></OPERATION-PROTOTYPE>
              </OPERATIONS>
            </CLIENT-SERVER-INTERFACE>
`;
  });
  model.compEntries.forEach(function(c){
    var ports="";
    c.ports.provided.forEach(function(p){var i=model.ifaceEntries.find(function(x){return x.name===p.iface});ports+=`                <P-PORT-PROTOTYPE>
                  <SHORT-NAME>${p.short}</SHORT-NAME>${i?`
                  <PROVIDED-INTERFACE-TREF DEST="${destForType(i.type)}">${pkgs.interfaces}/${i.short}</PROVIDED-INTERFACE-TREF>`:""}
                </P-PORT-PROTOTYPE>
`});
    c.ports.required.forEach(function(p){var i=model.ifaceEntries.find(function(x){return x.name===p.iface});ports+=`                <R-PORT-PROTOTYPE>
                  <SHORT-NAME>${p.short}</SHORT-NAME>${i?`
                  <REQUIRED-INTERFACE-TREF DEST="${destForType(i.type)}">${pkgs.interfaces}/${i.short}</REQUIRED-INTERFACE-TREF>`:""}
                </R-PORT-PROTOTYPE>
`});
    compTypeXml+=`            <ADAPTIVE-APPLICATION-SW-COMPONENT-TYPE>
              <SHORT-NAME>${c.short}Type</SHORT-NAME>${ports?`
              <PORTS>
${ports}              </PORTS>`:""}
            </ADAPTIVE-APPLICATION-SW-COMPONENT-TYPE>
`;
    prototypeXml+=`                <SW-COMPONENT-PROTOTYPE><SHORT-NAME>${c.short}</SHORT-NAME><TYPE-TREF DEST="ADAPTIVE-APPLICATION-SW-COMPONENT-TYPE">${pkgs.componentTypes}/${c.short}Type</TYPE-TREF></SW-COMPONENT-PROTOTYPE>
`;
  });
  model.connEntries.forEach(function(c){connectorXml+=`                <ASSEMBLY-SW-CONNECTOR>
                  <SHORT-NAME>${c.short}</SHORT-NAME>
                  <PROVIDER-IREF><CONTEXT-COMPONENT-REF DEST="SW-COMPONENT-PROTOTYPE">${pkgs.compositions}/${safeEsc(profile.compositionName)}/${c.src.short}</CONTEXT-COMPONENT-REF><TARGET-P-PORT-REF DEST="P-PORT-PROTOTYPE">${pkgs.componentTypes}/${c.src.short}Type/${c.srcPort.short}</TARGET-P-PORT-REF></PROVIDER-IREF>
                  <REQUESTER-IREF><CONTEXT-COMPONENT-REF DEST="SW-COMPONENT-PROTOTYPE">${pkgs.compositions}/${safeEsc(profile.compositionName)}/${c.dst.short}</CONTEXT-COMPONENT-REF><TARGET-R-PORT-REF DEST="R-PORT-PROTOTYPE">${pkgs.componentTypes}/${c.dst.short}Type/${c.dstPort.short}</TARGET-R-PORT-REF></REQUESTER-IREF>
                </ASSEMBLY-SW-CONNECTOR>
`});
  if(profile.addDeployment)deploymentXml=`
        <AR-PACKAGE>
          <SHORT-NAME>Deployments</SHORT-NAME>
          <ELEMENTS>
            <ADAPTIVE-APPLICATION-SW-COMPONENT-TYPE><SHORT-NAME>DeploymentManifest</SHORT-NAME></ADAPTIVE-APPLICATION-SW-COMPONENT-TYPE>
          </ELEMENTS>
        </AR-PACKAGE>`;
  return{xml:`<?xml version="1.0" encoding="UTF-8"?>
<!--
  Synthetic, non-proprietary ARXML generated by SysCanvas Editor v1.1.0 (implemented by Vibe Code through AI)
  Export profile: ${profile.label}
...
<AUTOSAR xmlns="http://autosar.org/schema/r4.0"
  ...
  <AR-PACKAGES>
    <AR-PACKAGE>
      <SHORT-NAME>${pkgs.root}</SHORT-NAME>
      <AR-PACKAGES>
        <AR-PACKAGE>
...
${compTypeXml}          </ELEMENTS>
        ...
        <AR-PACKAGE>
          <SHORT-NAME>${safeEsc(profile.compositionPackage)}</SHORT-NAME>
          <ELEMENTS>
            <COMPOSITION-SW-COMPONENT-TYPE>
              <SHORT-NAME>${safeEsc(profile.compositionName)}</SHORT-NAME>
              <COMPONENTS>
...
function setExportProfile(v){currentExportProfile=EXPORT_PROFILES[v]?v:"demo",updateExportWarningsUI()}
function updateExportWarningsUI(){
  var btn=document.getElementById("exportWarnBtn");if(!btn)return;
  var res=validateExportModel(EXPORT_PROFILES[currentExportProfile]||EXPORT_PROFILES.demo),count=res.warnings.length+res.errors.length;
  lastExportWarnings=res.warnings.slice();
  btn.textContent=`⚠ ${count} export warning${count===1?"":"s"}`;
  btn.classList.toggle("show",count>0);
}
function showExportWarnings(){
  var res=validateExportModel(EXPORT_PROFILES[currentExportProfile]||EXPORT_PROFILES.demo),items=[];
  res.errors.forEach(function(e){items.push(`<li style="color:var(--coral)"><strong>Error:</strong> ${escH(e)}</li>`)});
  res.warnings.forEach(function(w){items.push(`<li><strong>Warning:</strong> ${escH(w)}</li>`)});
  document.getElementById("modal").classList.add("vis");
  document.getElementById("modalC").innerHTML=`<h3>Pre-export checks (${items.length})</h3>${items.length?`<ul style="padding-left:18px;display:grid;gap:6px;font-size:11px;color:var(--t2)">${items.join("")}</ul>`:'<p style="font-size:11px;color:var(--teal)">No warnings. Export should be consistent.</p>'}<div style="display:flex;gap:6px;margin-top:14px"><button class="btn btn-g" onclick="document.getElementById('modal').classList.remove('vis')">Close</button></div>`;
}
function downloadARXML(){
  if(!allComps().length)return void alert("Add at least one component first.");
  var out=generateARXML();
  if(out.errors.length){showExportWarnings();return void alert("Resolve export errors before downloading ARXML.")}
  if(out.warnings.length&&!confirm("Export has warnings. Continue download?")){showExportWarnings();return}
  var e=new Blob([out.xml],{type:"application/xml"}),n=document.createElement("a");n.href=URL.createObjectURL(e),n.download="syscanvas_generated.arxml",n.click(),URL.revokeObjectURL(n.href),updateExportWarningsUI()
}
...
"undefined"==typeof d3&&(document.getElementById("rpBody").innerHTML='<div style="padding:30px 14px;text-align:center;color:var(--coral);font-size:11px">Failed to load D3.js. Check internet connection.</div>'),renderVehicle(),renderPanel(),updStatus();var _ep=document.getElementById("exportProfile");_ep&&(_ep.value=currentExportProfile),updateExportWarningsUI();