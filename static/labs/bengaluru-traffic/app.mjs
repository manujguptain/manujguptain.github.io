...
try{
  ...
  legend.innerHTML='<span>Light: good time to travel</span><span>Busy: allow extra time</span><span>Local peak: area-specific evidence</span>';
  ...
  function zoneWindowNote(zoneId,windowId){const p=zoneProfile(zoneId);if(!p||!isLocalPeak(zoneId,windowId))return'';return `Published local timing evidence marks this as part of a peak-sensitive period for ${p.junctions?.slice(0,3).join(', ')||'this area'}.`;}
  ...
  function render(){
    ...
    const peaks=profile?.localPeaks?.length?`<p class="zone-note"><strong>Published local peak periods:</strong> ${profile.localPeaks.join(' and ')}. <span class="evidence">Evidence: ${profile.evidence}.</span></p>`:`<p class="zone-note"><strong>Local peak timing:</strong> not reliable enough yet, so the Bengaluru-wide timing curve is used.</p>`;
